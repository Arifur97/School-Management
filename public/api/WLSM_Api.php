...
class WLSM_Api {
	...
	public static function student_profile( $request ) {
		...
		try {
			...
			$other_details  = array(
				...
				'photo'   => esc_url( wp_get_attachment_url( $student->photo_id ) ),
			...
			if ( count( $invoices ) ) {
				foreach ( $invoices as $row ) {
					...
					$invoices_data[] = array(
						...
						'payable_display' => esc_html( WLSM_Config::get_money_text( $row->payable ) ),
						'paid'            => esc_html( WLSM_Config::sanitize_money( $row->paid ) ),
						'paid_display'    => esc_html( WLSM_Config::get_money_text( $row->paid ) ),
						'due'             => esc_html( WLSM_Config::sanitize_money( $due ) ),
						'due_display'     => esc_html( WLSM_Config::get_money_text( $due ) ),
	...
	public static function fee_invoice( $request ) {
		...
		try {
			...
			$response_data['invoice'] = array(
				...
				'fees_due_display'        => esc_html( WLSM_Config::get_money_text( $due ) ),
			...
			if ( count( $payments ) ) {
				foreach ( $payments as $key => $row ) {
					...
					$payments_data[] = array(
						...
						'amount_display' => esc_html( WLSM_Config::get_money_text( $row->amount ) ),
	...
	public static function student_payment( $request ) {
		...
		try {
			...
			$response_data['payment'] = array(
				...
				'amount_display'    => esc_html( WLSM_Config::get_money_text( $payment->amount ) ),
	...
	// Parent - Students.
	public static function parent_students() {
		$user_id = get_current_user_id();
